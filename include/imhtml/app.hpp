...
public:
    ...
    // Density of the framebuffer relative to the layout grid. The shell only
    // needs it to rasterize fonts at the right size; it defaults to whatever the
    // ImGui backend reported, so a host on any windowing system can ignore it.
    // Pass a value explicitly when the backend does not set
    // io.DisplayFramebufferScale (or when it changes after a display switch).
    bool initialize(const std::string& font_path, float display_scale = 0.0f);
    bool initialize(std::span<const unsigned char> font_data, float display_scale = 0.0f);
    void frame();
    void shutdown();
