...
Context::Context() {
  config_.BaseFontSize = 16;
  config_.LoadCSS = DefaultFileLoader;
}
...
Config& Context::config() { return config_stack_.empty() ? config_ : config_stack_.back(); }
const Config& Context::config() const { return config_stack_.empty() ? config_ : config_stack_.back(); }

void Context::push_config(const Config& config) { config_stack_.push_back(config); }

void Context::pop_config() {
  assert(!config_stack_.empty());
  if (!config_stack_.empty()) config_stack_.pop_back();
}

void Context::register_custom_element(const std::string& tag, CustomElementDrawFunction draw) {
  draw_elements_[tag] = std::move(draw);
}

void Context::register_custom_element_html(const std::string& tag, CustomElementHtmlFunction render) {
  html_elements_[tag] = std::move(render);
}

void Context::unregister_custom_element(const std::string& tag) { draw_elements_.erase(tag); }

void Context::unregister_custom_element_html(const std::string& tag) { html_elements_.erase(tag); }

const CustomElementDrawFunction* Context::find_custom_element(const std::string& tag) const {
  const auto found = draw_elements_.find(tag);
  return found == draw_elements_.end() ? nullptr : &found->second;
}

const CustomElementHtmlFunction* Context::find_custom_element_html(const std::string& tag) const {
  const auto found = html_elements_.find(tag);
  return found == html_elements_.end() ? nullptr : &found->second;
}

bool Context::has_custom_element(const std::string& tag) const { return find_custom_element(tag) != nullptr; }

namespace {
// Thread-local so two threads can drive two documents; a single context is
// still single-threaded.
thread_local Context* current_context = nullptr;
}  // namespace

Context& CurrentContext() {
  return current_context == nullptr ? Context::Default() : *current_context;
}

void SetCurrentContext(Context& context) { current_context = &context; }

ContextScope::ContextScope(Context& context) : previous_(current_context) { current_context = &context; }

ContextScope::~ContextScope() { current_context = previous_; }

namespace {

const Config* getCurrentConfigPtr() { return &CurrentContext().config(); }
...
void CustomElement::draw_background(litehtml::uint_ptr hdc, litehtml::pixel_t x, litehtml::pixel_t y,
  ...
  if (const CustomElementDrawFunction* draw = CurrentContext().find_custom_element(this->tag)) {
    ImVec2 cursor = ImGui::GetCursorScreenPos();
    const auto to_screen_rect = [&cursor](const litehtml::position& position) {
      return Rect(cursor + ImVec2(position.x, position.y),
                    cursor + ImVec2(position.x + position.width, position.y + position.height));
    };
    (*draw)(to_screen_rect(border_box), this->attributes);
...
// Layout runs on the logical grid while the framebuffer can be denser
// (fractional display scale), so box edges land on sub-pixel positions. Two
// boxes inset from each other by the same amount then rasterize with different
// edge coverage, and a centred knob reads as sitting high or low. Snapping box
// fills to the device grid keeps equal insets equal.
ImVec2 SnapToDevicePixels(const ImVec2& point) {
  const ImVec2 scale = ImGui::GetIO().DisplayFramebufferScale;
  const float scale_x = scale.x > 0.0f ? scale.x : 1.0f;
  const float scale_y = scale.y > 0.0f ? scale.y : 1.0f;
  return ImVec2(std::round(point.x * scale_x) / scale_x, std::round(point.y * scale_y) / scale_y);
}

// Clip rectangles round outward instead. A clip that lands mid-pixel costs the
// shape inside it its outermost row of coverage, and once the fill boxes snap,
// a box can legitimately reach a hair past a clip edge that was computed from
// the unsnapped layout.
ImVec2 GrowToDevicePixels(const ImVec2& point, const bool up) {
  const ImVec2 scale = ImGui::GetIO().DisplayFramebufferScale;
  const float scale_x = scale.x > 0.0f ? scale.x : 1.0f;
  const float scale_y = scale.y > 0.0f ? scale.y : 1.0f;
  if (up) {
    return ImVec2(std::ceil(point.x * scale_x) / scale_x, std::ceil(point.y * scale_y) / scale_y);
  }
  return ImVec2(std::floor(point.x * scale_x) / scale_x, std::floor(point.y * scale_y) / scale_y);
}

// Consecutive corner arcs share their meeting point, and a radius equal to half
// the box turns whole edges into a single shared point. Those duplicates give
// ImGui's antialiasing a zero-length segment to compute a normal from, which
// shows up as a spike on the outline, so drop them before filling or stroking.
void DedupePath(ImDrawList* draw_list) {
  ImVector<ImVec2>& path = draw_list->_Path;
  const auto same = [](const ImVec2& a, const ImVec2& b) {
  ...
  for (int index = 0; index < path.Size; ++index) {
    if (kept > 0 && same(path[index], path[kept - 1])) continue;
    path[kept++] = path[index];
  }
  // The shape closes on its own; a repeated first point is a duplicate too.
  if (kept > 1 && same(path[kept - 1], path[0])) --kept;
  path.Size = kept;
}
...
class BrowserContainer : public litehtml::document_container {
 private:
  ...
  ImVec2 frame_origin = ImVec2(0, 0);
  ImVec2 frame_viewport = ImVec2(0, 0);
  ImDrawList* frame_draw_list = nullptr;
  bool frame_hovered = false;
  std::uint64_t frame_text_runs = 0;
  ...
  void draw_scrollbars(const std::vector<ScrollState>& states,
                       const std::vector<ScrollbarVisualState>& visuals, const ImVec2& origin) const {
    ImDrawList* draw_list = frame_draw_list;
 ...
 public:
  ...
  // The container is called from litehtml during parsing, layout, hit testing
  // and painting. Resolving the ImGui window on every one of those callbacks
  // both costs time and makes the container unusable outside a live window, so
  // the frame state is captured once and read from these members afterwards.
  void begin_frame(const ImVec2& origin, const ImVec2& viewport, ImDrawList* draw_list, bool hovered) {
    frame_origin = origin;
    frame_viewport = viewport;
    frame_draw_list = draw_list;
    frame_hovered = hovered;
    frame_text_runs = 0;
    bottomRight = ImVec2(0, 0);
  }

  const ImVec2& origin() const { return frame_origin; }
  ImDrawList* draw_list() const { return frame_draw_list; }
  std::uint64_t text_runs() const { return frame_text_runs; }
  ...
  void apply_requested_cursor() const {
    if (requested_cursor != ImGuiMouseCursor_Arrow && frame_hovered) {
  ...
  virtual litehtml::pixel_t text_width(const char* text, litehtml::uint_ptr hFont) override {
    ...
    const bool bold = rf->Style == FontStyle::Bold || rf->Style == FontStyle::BoldItalic;
    return (litehtml::pixel_t)(size.x + (bold ? 1.0f : 0.0f));
  }
  ...
  virtual void draw_text(litehtml::uint_ptr hdc, const char* text, litehtml::uint_ptr hFont, litehtml::web_color color,
    ...
    ++frame_text_runs;
    ImVec2 p = frame_origin + ImVec2(pos.x, pos.y);
    ImU32 col = IM_COL32(color.red, color.green, color.blue, color.alpha);

    frame_draw_list->AddText(rf->Font, rf->Size, p, col, text);
    // A variable font loaded through ImGui's stb_truetype path exposes its
    // default instance only. Keep CSS font-weight meaningful even when the
    // configured family therefore resolves bold to the same ImFont: a second
    // sub-pixel-shifted impression produces a conventional synthetic bold.
    if (rf->Style == FontStyle::Bold || rf->Style == FontStyle::BoldItalic) {
      frame_draw_list->AddText(rf->Font, rf->Size, p + ImVec2(0.9f, 0.0f), col, text);
    }

    // litehtml has already measured this text and supplied its layout box in
    // `pos`. Re-measuring it here was pure per-frame paint overhead.
    push_bottom_right(ImVec2(pos.x + pos.width, pos.y + pos.height));
  }
  ...
  LayerGeometry get_layer_geometry(const litehtml::background_layer& layer) const {
    ImVec2 screen_pos = frame_origin;

    LayerGeometry g;
    g.border_min = SnapToDevicePixels(screen_pos + ImVec2((float)layer.border_box.x, (float)layer.border_box.y));
    g.border_max = SnapToDevicePixels(screen_pos + ImVec2((float)(layer.border_box.x + layer.border_box.width),
                                                          (float)(layer.border_box.y + layer.border_box.height)));
    g.clip_min = GrowToDevicePixels(screen_pos + ImVec2((float)layer.clip_box.x, (float)layer.clip_box.y), false);
    g.clip_max = GrowToDevicePixels(screen_pos + ImVec2((float)(layer.clip_box.x + layer.clip_box.width),
                                                        (float)(layer.clip_box.y + layer.clip_box.height)),
                                    true);

    // litehtml clamps the radii against the unsnapped box, which can be up to a
    // pixel larger than the one drawn; re-clamp so opposite arcs never overlap.
    const float max_radius =
        std::min(g.border_max.x - g.border_min.x, g.border_max.y - g.border_min.y) / 2.0f;
    g.tl = std::min((float)layer.border_radius.top_left_x, max_radius);
    g.tr = std::min((float)layer.border_radius.top_right_x, max_radius);
    g.br = std::min((float)layer.border_radius.bottom_right_x, max_radius);
    g.bl = std::min((float)layer.border_radius.bottom_left_x, max_radius);
    return g;
  }

  // A layer whose clip box already covers the border box needs no scissor: the
  // fill is bounded by the box itself. Pushing one anyway cuts the antialiased
  // fringe that sits just outside the geometry, which turns rounded corners
  // into hard, faceted edges — very visible on pill-shaped controls.
  static bool layer_clip_covers_box(const LayerGeometry& g) {
    constexpr float slack = 0.01f;
    return g.clip_min.x <= g.border_min.x + slack && g.clip_min.y <= g.border_min.y + slack &&
           g.clip_max.x >= g.border_max.x - slack && g.clip_max.y >= g.border_max.y - slack;
  }

  virtual void draw_list_marker(litehtml::uint_ptr hdc, const litehtml::list_marker& marker) override {
    ImDrawList* draw_list = frame_draw_list;
    ImVec2 center = frame_origin +
    ...
    switch (marker.marker_type) {
      ...
      case litehtml::list_style_type_square: {
        ImVec2 p_min = frame_origin + ImVec2(marker.pos.x, marker.pos.y);
  ...
  virtual void draw_solid_fill(litehtml::uint_ptr hdc, const litehtml::background_layer& layer,
    ...
    const bool clipped = !layer_clip_covers_box(lgm);
    if (clipped) {
      PushSafeClipRect(draw_list, lgm.clip_min, lgm.clip_max);
    }

    // AddRectFilled caps its rounding at half the shorter side minus one pixel,
    // which leaves a flat segment on the ends of a fully rounded pill. Draw the
    // arcs ourselves whenever the radius is close enough to that cap for the
    // difference to show.
    const float half_extent = std::min(lgm.border_max.x - lgm.border_min.x, lgm.border_max.y - lgm.border_min.y) / 2.0f;
    const bool uniform_radius = lgm.tl == lgm.tr && lgm.tr == lgm.br && lgm.br == lgm.bl;
    if (uniform_radius && lgm.tl < half_extent - 1.0f) {
  ...
  void draw_linear_gradient_impl(const LayerGeometry& lgm,
    ...
    const ImVec2 screen_pos = frame_origin;
  ...
  virtual void on_mouse_event(const litehtml::element::ptr& el, litehtml::mouse_event event) override {
    if (el != nullptr && frame_hovered) {
      if (event == litehtml::mouse_event_enter) {
        tooltip.clear();
        for (litehtml::element::ptr current = el; current != nullptr; current = current->parent()) {
          if (const char* attr = current->get_attr("title"); attr != nullptr) {
    ...
    } else if (event == litehtml::mouse_event_leave && !frame_hovered) {
      if (!hovered_id.empty()) queue_element_event("mouse-exit", hovered_id);
      hovered_id.clear();
    }
  ...
  virtual void draw_borders(litehtml::uint_ptr hdc, const litehtml::borders& borders,
    ...
    ImVec2 base_pos = frame_origin;
    // Snapped like the background fills so a border stays flush with the box
    // it outlines.
    ImVec2 top_left = SnapToDevicePixels(base_pos + ImVec2(draw_pos.x, draw_pos.y));
    ImVec2 bottom_right =
        SnapToDevicePixels(base_pos + ImVec2(draw_pos.x + draw_pos.width, draw_pos.y + draw_pos.height));
    ImVec2 top_right = ImVec2(bottom_right.x, top_left.y);
    ImVec2 bottom_left = ImVec2(top_left.x, bottom_right.y);

    auto* draw_list = frame_draw_list;
  ...
  virtual bool on_element_click(const litehtml::element::ptr& element) override {
    for (auto current = element; current; current = current->parent()) {
      ...
      if (tag != nullptr && std::strcmp(tag, "input") == 0 &&
          (std::strcmp(current->get_attr("type", ""), "text") == 0 ||
           std::strcmp(current->get_attr("type", ""), "number") == 0)) {
        return true;
      }
    }
    for (auto current = element; current; current = current->parent()) {
      ...
      if (tag != nullptr && std::strcmp(tag, "input") == 0 &&
          std::strcmp(current->get_attr("type", ""), "color") == 0) {
        return true;
      }
    }
    for (auto current = element; current; current = current->parent()) {
    ...
    for (auto current = element; current; current = current->parent()) {
      const char* tag = current->get_tagName();
      if (tag != nullptr && CurrentContext().has_custom_element(tag)) {
        return true;
      }
    }
  ...
  virtual void set_clip(const litehtml::position& pos, const litehtml::border_radiuses&) override {
    ImDrawList* draw_list = frame_draw_list;
    if (draw_list == nullptr) return;
    const ImVec2 cursor = frame_origin;
    const ImVec2 clip_min = GrowToDevicePixels(cursor + ImVec2(pos.x, pos.y), false);
    const ImVec2 clip_max = GrowToDevicePixels(cursor + ImVec2(pos.x + pos.width, pos.y + pos.height), true);
    PushSafeClipRect(draw_list, clip_min, clip_max);
  }
  ...
  virtual void get_viewport(litehtml::position& client) const override {
    client.x = 0;
    client.y = 0;
    client.width = frame_viewport.x;
    client.height = frame_viewport.y;
  }
  ...
  virtual litehtml::element::ptr create_element(const char* tag_name, const litehtml::string_map& attributes,
    ...
    if (std::strcmp(tag_name, "input") == 0) {
      ...
      if (type != attributes.end() &&
          (type->second == "checkbox" || type->second == "range" ||
           type->second == "text" || type->second == "color")) {
        return nullptr;
      }
    }
    if (CurrentContext().has_custom_element(tag_name)) {
  ...
  virtual void get_media_features(litehtml::media_features& media) const override {
    media.color = 8;
    media.resolution = 96;
    media.width = frame_viewport.x;
    media.height = frame_viewport.y;
    media.device_width = frame_viewport.x;
    media.device_height = frame_viewport.y;
...
// Diagnostics default to stderr so the environment-variable tracing keeps
// working out of the box; a host that installs Config::Log takes over entirely.
void DefaultLogSink(const LogLevel level, const char* message) {
  ...
  std::fprintf(stderr, "[ImHTML %s] %s\n", label, message);
}

}  // namespace

void LogMessage(const LogLevel level, const std::string& message) {
  if (const auto& sink = GetConfig()->Log) {
    sink(level, message.c_str());
    return;
  }
  DefaultLogSink(level, message.c_str());
}

void LogPrintf(const LogLevel level, const char* format, ...) {
  ...
  if (length > 0) {
    ...
    // Call sites were written for fprintf and often end in a newline; the sink
    // owns line breaks now.
...
Config* GetConfig() { return &CurrentContext().config(); }
void SetConfig(const Config& newConfig) { CurrentContext().config() = newConfig; }
void PushConfig(const Config& config) { CurrentContext().push_config(config); }
void PopConfig() {
  CurrentContext().pop_config();
}

void RegisterCustomElement(const char* tagName, CustomElementDrawFunction draw) {
  CurrentContext().register_custom_element(tagName, std::move(draw));
}

void UnregisterCustomElement(const char* tagName) {
  CurrentContext().unregister_custom_element(tagName);
}

void RegisterCustomElementHtml(const char* tagName, CustomElementHtmlFunction render_html) {
  CurrentContext().register_custom_element_html(tagName, std::move(render_html));
}

void UnregisterCustomElementHtml(const char* tagName) { CurrentContext().unregister_custom_element_html(tagName); }
...
struct CanvasState {
  ...
  bool scroll_states_valid = false;
...
// litehtml's document calls back into its container while it is being
// destroyed, so a document handed out to a caller has to keep its container
// alive for as long as the caller holds it.
std::shared_ptr<litehtml::document> OwnDocument(std::shared_ptr<litehtml::document> document,
                                                std::shared_ptr<BrowserContainer> container) {
  litehtml::document* raw = document.get();
  return std::shared_ptr<litehtml::document>(
      raw, [document = std::move(document), container = std::move(container)](litehtml::document*) mutable {
        document.reset();
        container.reset();
      });
}

// Parsing evaluates media queries, so the container needs a viewport before the
// document exists. Outside a live ImGui window only the requested width is known.
ImVec2 CurrentViewport(float width) {
  ImVec2 available(0.0f, 0.0f);
  if (ImGui::GetCurrentContext() != nullptr && ImGui::GetCurrentWindowRead() != nullptr) {
    available = ImGui::GetContentRegionAvail();
  }
  return ImVec2(width > 0.0f ? width : available.x, available.y);
}

std::shared_ptr<litehtml::document> ParseDocument(const char* id, const char* html, float width) {
  if (id == nullptr || id[0] == '\0' || html == nullptr) return {};
  ...
  if (it == states.end()) {
    ...
    container->begin_frame(ImVec2(0.0f, 0.0f), CurrentViewport(width), nullptr, false);
    it = states
             .emplace(id, CanvasState{
                              .container = container,
                              .doc = OwnDocument(
                                  litehtml::document::createFromString(ExpandCustomElements(html), container.get()),
                                  container),
                              .html = html,
    ...
    it->second.container->begin_frame(ImVec2(0.0f, 0.0f), CurrentViewport(width), nullptr, false);
    it->second.doc = OwnDocument(
    ...
    it->second.scroll_states_valid = false;
...
const std::vector<ScrollState>& FrameScrollStates(const char* id) {
  static const std::vector<ScrollState> empty;
  if (id == nullptr) return empty;
  auto& states = canvas_states();
  const auto found = states.find(id);
  return found == states.end() ? empty : found->second.scroll_states;
}
...
bool RenderDocument(const char* id, float width, std::string* clickedURL) {
  if (id == nullptr || id[0] == '\0' || ImGui::GetCurrentContext() == nullptr ||
  ...
  int render_width = width > 0 ? (int)width : (int)document_available.x;
  int render_height = (int)document_available.y;
  state.container->begin_frame(document_origin,
                               ImVec2(static_cast<float>(render_width), document_available.y),
                               ImGui::GetWindowDrawList(), ImGui::IsWindowHovered());
  ...
  state.last_frame_relayout = did_relayout;
  if (did_relayout) {
    const auto layout_started = std::chrono::steady_clock::now();
    state.doc->render(render_width);
    state.last_layout_ms =
        std::chrono::duration<double, std::milli>(std::chrono::steady_clock::now() - layout_started).count();
    ++state.layout_count;
    state.layout_width = render_width;
    state.layout_height = render_height;
    state.layout_dirty = false;
  }
  // litehtml culls leaves against this rectangle, so it has to describe what is
  // actually on screen. Passing the full document height disabled that culling.
  litehtml::position clip(0, 0, render_width, (int)document_available.y);
  const auto paint_started = std::chrono::steady_clock::now();
  state.container->apply_requested_cursor();
  state.doc->draw(0, 0, 0, &clip);
  state.last_paint_ms =
      std::chrono::duration<double, std::milli>(std::chrono::steady_clock::now() - paint_started).count();

  const ImVec2 mouse = ImGui::GetMousePos();
  const auto x = mouse.x - document_origin.x;
  const auto y = mouse.y - document_origin.y;

  std::vector<ScrollState>& scroll_states = state.scroll_states;
  if (did_relayout || !state.scroll_states_valid) {
    CollectScrollStates(state.doc, scroll_states);
    state.scroll_states_valid = true;
  }
  ...
  const bool pointer_available = !scrollbar_input && !state.pointer_blocked;
  const bool lbutton_down = pointer_available && ImGui::IsMouseDown(ImGuiMouseButton_Left);
  const bool lbutton_up = pointer_available && ImGui::IsMouseReleased(ImGuiMouseButton_Left);
  if (lbutton_down) {
  ...
  if (!state.container->get_tooltip().empty()) {
    // Keep title tooltips visually consistent with the app's rounded dark UI.
    // ImGui renders tooltips as popup windows, so these scoped overrides apply
    // only to the tooltip and do not affect the surrounding document.
    ImGui::PushStyleVar(ImGuiStyleVar_WindowPadding, ImVec2(10.0f, 7.0f));
    ImGui::PushStyleVar(ImGuiStyleVar_WindowRounding, 10.0f);
    ImGui::PushStyleVar(ImGuiStyleVar_PopupRounding, 10.0f);
    ImGui::PushStyleVar(ImGuiStyleVar_WindowBorderSize, 0.5f);
    ImGui::PushStyleColor(ImGuiCol_PopupBg, ImVec4(0.105f, 0.110f, 0.135f, 0.98f));
    ImGui::PushStyleColor(ImGuiCol_Border, ImVec4(0.255f, 0.265f, 0.295f, 1.0f));
    ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(0.950f, 0.945f, 0.980f, 1.0f));
...
void SetDocumentPointerBlocked(const char* id, const bool blocked) {
  if (id == nullptr) return;
  auto& states = canvas_states();
  const auto found = states.find(id);
  if (found != states.end()) found->second.pointer_blocked = blocked;
}
...
DocumentRenderStats GetDocumentRenderStats(const char* id) {
  DocumentRenderStats result;
  if (id == nullptr) return result;
  const auto found = canvas_states().find(id);
  if (found == canvas_states().end()) return result;
  result.frames = found->second.frame_count;
  result.parses = found->second.parse_count;
  result.layouts = found->second.layout_count;
  result.paints = found->second.paint_count;
  result.text_runs = found->second.container ? found->second.container->text_runs() : 0;
  result.last_layout_ms = found->second.last_layout_ms;
  result.last_paint_ms = found->second.last_paint_ms;
  result.last_frame_relayout = found->second.last_frame_relayout;
  return result;
}
};  // namespace ImHTML
