...
bool IsExternalURL(const std::string& url) {
    ...
    // Never hand executable/document pseudo-URLs to the host shell. Normal
    // web, mail, phone, file, and custom application schemes are external.
...
float RangeThumbWidth(float height) {
    // Browser range thumbs are commonly wider than their track height. Keep
    // this as one shared metric for both hit-testing and painting so the pill
    // never jumps when clicked near an endpoint.
...
public:
    NativeControlClipScope(const std::shared_ptr<litehtml::document>& document,
        ...
        document->get_scroll_states(scroll_states);
...
bool GetSelectScrollbarGeometry(const std::shared_ptr<litehtml::element>& select,
    ...
    const float thickness = litehtml::scrollbar_size_pixels(width);
...
void HtmlApplication::set_stylesheet_provider(std::function<std::string()> provider) { stylesheet_provider_ = std::move(provider); }

void HtmlApplication::register_shell(std::string html) { shell_html_ = std::move(html); }

void HtmlApplication::register_fragment(const std::string& name, FragmentHtmlProvider html_provider) {
    fragments_[name] = std::move(html_provider);
}

void HtmlApplication::register_html_control(std::unique_ptr<HtmlControl> control) {
    if (control) html_controls_.push_back(std::move(control));
}

HtmlApplication::EventListenerId HtmlApplication::on(const std::string& event,
...
void HtmlApplication::off(const EventListenerId listener_id) {
...
bool HtmlApplication::initialize(UiApplication* application) {
    application_ = application;
    return !shell_html_.empty();
}

std::vector<std::shared_ptr<litehtml::element>> HtmlApplication::query_selector_all(
...
std::shared_ptr<litehtml::element> HtmlApplication::query_selector(const std::string& selector) const {
    if (!doc_ || !doc_->root() || selector.empty()) return {};
    return doc_->root()->select_one(selector.c_str());
}

void HtmlApplication::append_html(const std::string& selector, const std::string& html,
    ...
    doc_->refresh_styles();
    doc_->invalidate_layout_cache(target);
    MarkDocumentLayoutDirty("app");
}

bool HtmlApplication::has_value(const std::string& id) const {
    return !id.empty() && values_.find(id[0] == '#' ? id.substr(1) : id) != values_.end();
}

void HtmlApplication::set_value(const std::string& id, const std::string& value) {
    if (id.empty()) return;
...
void HtmlApplication::set_checked(const std::string& id, const bool checked_value) {
    if (id.empty()) return;
    const std::string key = id[0] == '#' ? id.substr(1) : id;
...
void HtmlApplication::set_options(const std::string& id, std::vector<std::string> options) {
    if (!id.empty()) options_[id[0] == '#' ? id.substr(1) : id] = std::move(options);
}

const std::vector<std::string>& HtmlApplication::options(const std::string& id) const {
...
std::string HtmlApplication::value(const std::string& selector) const {
...
double HtmlApplication::value_as_number(const std::string& selector, const double fallback) const {
    return ParseRangeNumber(value(selector).c_str(), fallback);
}

bool HtmlApplication::checked(const std::string& selector) const {
    const auto element = query_selector(selector);
    if (element) {
...
std::string HtmlApplication::state_value(const std::string& key, const std::string& fallback) const {
    if (!has_value(key)) return fallback;
    return value(key);
}

void HtmlApplication::set_state(const std::string& key, const std::string& value) {
    set_value(key, value);
}

bool HtmlApplication::dispatch_event(const std::string& id, const std::string& event,
    ...
    // A snapshot keeps dispatch deterministic if a listener removes itself
    // or registers another listener while handling the event.
...
bool HtmlApplication::DispatchHtmlControlEvent(const std::string& id, const std::string& event) {
...
void HtmlApplication::NotifyHtmlControlTextFocus(const std::string& id) {
...
bool HtmlApplication::HandleHtmlControlTextKey(const std::string& id) {
    if (!application_) return false;
    for (const auto& control : html_controls_) {
...
bool HtmlApplication::NotifyHtmlControlTextChanged(const std::string& id, const std::string& value) {
    if (!application_) return false;
    bool handled = false;
    for (const auto& control : html_controls_) {
...
void HtmlApplication::RequestStructuralRebuild() { needs_rebuild_ = true; }

void HtmlApplication::mark_dirty() { RequestStructuralRebuild(); }

void HtmlApplication::set_list(std::string id, std::vector<inja::json> items) {
    if (id.empty()) return;
    const auto found = lists_.find(id);
    if (found != lists_.end() && found->second == items) return;

    // Selection is presentation state. If the list has the same rows and
    // only their selected flags changed, update the existing row classes in
    // place so list scrolling, input focus, and the rest of the document
    // survive the publication.
    ...
    // Lists belonging to an inactive fragment are still retained for the next
    // switch, but rebuilding the visible document just to prepare them would
    // discard layout/scroll state for no visual benefit. A fragment switch will
    // request its own structural rebuild when that list becomes visible.
    if (!doc_ || !doc_->root()) {
    ...
    // Patching the list's own subtree keeps the rest of the document - scroll
    // offsets included - exactly where it is, but the DOM must not change while
    // the document is being walked, so it happens at the top of the next frame.
    pending_list_updates_.insert(id);
}

namespace {

// Marks the two elements that stand in for the rows outside the window. They
// carry no styling of their own: the height is the whole point.
constexpr const char* kSpacerClass = "imhtml-virtual-spacer";
// Rows built before anything has been measured. Enough to fill a tall viewport
// and give the next frame a row height to work from.
constexpr std::size_t kProbeRows = 48;
// Rows kept beyond each edge of the viewport, so a scroll step shorter than a
// row never uncovers an unbuilt one.
...
std::string HtmlApplication::ExpandListRows(const std::string& id,
    ...
    if (window.uniform && items.size() > kProbeRows) {
        if (window.row_height > 0.0f) {
            // A window already measured against the live viewport.
            first = std::min(window.first, items.size() - 1);
            count = std::min(window.count, items.size() - first);
            if (count == 0) count = 1;
        } else {
            // Nothing measured yet. One screenful's worth of rows is what gives
            // the next frame something to measure; the real window follows it.
            count = kProbeRows;
        }
    }
    window.first = first;
    ...
    // Parse the item template once into the shared Inja cache. Rendering each
    // item below only evaluates that cached template with different data.
    PrepareHtmlTemplate(item_template);
    std::string result = SpacerHtml(window.lead);
    for (std::size_t index = first; index < first + count; ++index) {
        inja::json item = items[index];
        // Internal template metadata; application models do not need to carry
        // an artificial identifier just to create unique control IDs.
        item["__index"] = index;
        // Selection is optional for ordinary list rows. Supplying the default
        // here keeps shared templates safe when a page is rebuilt or a list
        // provider intentionally omits selection metadata.
        if (!item.contains("selected")) item["selected"] = false;
        // Trimmed: the template's own indentation would otherwise put a text
        // node between every pair of rows, and the row measurement counts the
...
bool HtmlApplication::UpdateListInPlace(const std::string& id) {
    ...
    // No render item means the list has never been laid out, so there is
    // nothing to preserve and the ordinary document build is the right path.
    if (!target->get_render_item()) return false;

    // Rebuilding the list's render subtree must not reset an enclosing scroll
    // container. Capture the live offsets before replacing the rows; the DOM
    // scroll elements themselves remain in place after the render tree is
    // rebuilt.
    struct ScrollOffset {
    ...
    std::vector<litehtml::scroll_state> scroll_states;
    doc_->get_scroll_states(scroll_states);
    for (const litehtml::scroll_state& state : scroll_states) {
    ...
    // The rows the cache pointed at have just been destroyed.
    element_cache_.clear();
    doc_->refresh_styles();
    ApplyStoredAriaSelection();
    // append_children_from_string creates render items for the direct list
    // children, but it does not run litehtml's full render-tree finalization.
    // That leaves nested flex/custom-element rows with stale or missing layout
    // boxes until some later document rebuild (for example, a tab switch).
    doc_->rebuild_render_tree();
    // Lay out the refreshed tree once now, before the host's paint pass, so
    // restored scroll positions and scrollbar geometry are ready immediately.
    // Do not mark the canvas dirty: that would repeat this same full layout in
    // RenderDocument and turn a one-update refresh into two expensive passes.
    doc_->render(static_cast<litehtml::pixel_t>(ImGui::GetContentRegionAvail().x));
    for (const ScrollOffset& offset : scroll_offsets) {
        offset.target->h_scroll(offset.left);
        offset.target->v_scroll(offset.top);
    }
    return true;
}

bool HtmlApplication::MeasureListRows(const std::string& id,
    ...
    // The row pitch is the distance between consecutive rows, not their height:
    // margins and border spacing are part of what the scroll offset has to be
    // divided by.
    ...
    // One item must produce exactly one row for a row index to mean anything.
    // A template that emits several elements per item, or rows of differing
    // heights, opts the list out of virtualization altogether.
...
void HtmlApplication::UpdateListWindows() {
    if (!doc_ || !doc_->root() || lists_.empty()) return;

    std::vector<litehtml::scroll_state> states;
    doc_->get_scroll_states(states);
    ...
    for (const auto& [id, items] : lists_) {
        ...
        if (!MeasureListRows(id, list_element)) {
            // The list just opted out: rebuild it whole, once.
            if (window.count != items.size()) pending_list_updates_.insert(id);
            continue;
        }

        // The nearest scrolling ancestor - or the list itself - is what decides
        // which rows are on screen. A list nobody scrolls has no window to
        // compute, and is left materialized as it is.
        const litehtml::scroll_state* scroller = nullptr;
        for (const litehtml::render_item* walk = list_render.get(); walk != nullptr && scroller == nullptr;
             walk = walk->parent_raw()) {
            for (const litehtml::scroll_state& state : states) {
        ...
        // Placements already carry the ancestors' scroll shifts, so the top of
        // the viewport in the list's own content space is the gap between them,
        // plus whatever the list scrolls itself.
...
void HtmlApplication::ApplyPendingListUpdates() {
    if (pending_list_updates_.empty()) return;
    // A click is a press and a release on the same element. Swapping a list's
    // rows in between destroys the pressed one, and litehtml then has nothing
    // to complete the click against - the click is simply lost. Rows therefore
    // never move while a button is held; a drag-scroll settles one frame after
    // the release instead.
...
std::string HtmlApplication::attribute(const std::string& element_id,
...
std::string HtmlApplication::enclosing_list(const std::string& element_id) const {
    if (element_id.empty() || !doc_ || !doc_->root()) return {};
...
HtmlApplication::ScrollMetrics HtmlApplication::scroll_metrics(const std::string& id) {
...
bool HtmlApplication::ApplyListSelectionPaint(const std::string& id,
    ...
    for (std::size_t index = 0; index < next.size(); ++index) {
        ...
        // Do not treat changed row content as paint-only. The template may
        // have emitted different text, controls, or structure for it.
    ...
    for (const auto& [row_id, selected] : changes) {
        // A row outside the list's window has no element to repaint. Its
        // selection lives in the item data and is applied when it is built.
        if (const auto row = doc_->root()->select_one("#" + row_id)) {
            row->set_class("selected", selected);
        }
    }
    // A class mutation changes selector matching, but does not change the
    // document structure or invalidate its layout. Refresh only the live
    // styles so the existing render tree paints the new row state immediately.
    doc_->refresh_styles();
    return true;
}

std::string HtmlApplication::RenderListTemplate(const std::map<std::string, std::string>&,
                                        std::string_view children, const HtmlElementContext& context) const {
    // A standard template has meaning only when its parent explicitly opts
    // into list rendering. This keeps the renderer independent of HTML tag
    // names: tables, cards, grids, and future custom containers all use the
    // same list contract.
    ...
    // Remember the template even when the list is still empty: an in-place
    // update later has no other way back to the markup it should expand.
    list_templates_[id->second] = std::string(children);
    return ExpandListRows(id->second, children);
}

void HtmlApplication::select(const std::string& group, const std::string& key) {
    ...
    if (group == "main") {
        ...
        // Commit at the next frame. The old document is kept intact for the
        // rest of the current render traversal.
        RequestStructuralRebuild();
        return;
    }

    set_state(group + "-selection", key);
    // Nested panels rebuild the same fresh document used by the top-level
    // switcher. Lazy switchers mount only the selected panel; the view-model
    // keeps all control values outside the DOM.
    RequestStructuralRebuild();
}

bool HtmlApplication::HasFragment(const std::string& name) const {
    return fragments_.find(name) != fragments_.end();
}

bool HtmlApplication::handle_interaction(const std::string& url) {
    ...
    // Anything with a URI scheme (http(s), mailto, tel, custom app links,
    // etc.) belongs to the operating system rather than the embedded app.
...
std::string HtmlApplication::BuildFragmentHtml(const std::string& name) const {
    const auto found = fragments_.find(name);
    return found == fragments_.end() || !found->second ? std::string() : found->second();
}

void HtmlApplication::SelectInitialFragment() {
...
void HtmlApplication::RebuildDocument() {
    ++rebuild_count_;
    // Inline styles belong to the old DOM. Persistent component values will
    // be reapplied through set_style() on the next frame.
    ...
    // A top-level fragment switch replaces the content subtree. Start from a fresh
    // shell and canvas so no renderer, hover target, scroll target, or cached
    // layout from the previous fragment can survive the transition.
    doc_.reset();
    content_slot_.reset();
    ResetDocument("app");
    ...
    if (!doc_) {
        ...
        const std::string expanded_shell = ExpandCustomElements(shell_html_, render_template);
        doc_ = ParseDocument("app", expanded_shell.c_str(), static_cast<float>(width));
        if (!doc_) {
            content_slot_.reset();
            return;
        }
        const std::string stylesheet = stylesheet_provider_ ? stylesheet_provider_() : std::string();
        if (!stylesheet.empty()) {
            doc_->add_stylesheet(stylesheet.c_str(), "", "");
        }
    ...
    ApplySwitchers();
    ApplyStoredAriaSelection();
    // Classes and appended fragments are applied after parsing. Reapply the
    // document stylesheets before the first render builds the layout tree.
    doc_->refresh_styles();
    doc_->rebuild_render_tree();
    MarkDocumentDirty("app");
}

void HtmlApplication::SyncCheckboxes() {
...
void HtmlApplication::SyncTextInputs() {
    if (!doc_ || !doc_->root()) return;
...
void HtmlApplication::DrawTextInputs(const ImVec2& document_origin) {
    if (!doc_ || !doc_->root()) return;
    ...
    // ImGui's SDL backend normally enables SDL text input while an ImGui
    // InputText widget owns focus. These fields are rendered by ImHTML, so
    // keep SDL's text-input stream alive while our own field is active.
    ...
    for (const auto& input : inputs) {
        ...
        // Selection and placeholder painting use the control's computed CSS
        // colors. litehtml does not expose a separate native selection or
        // placeholder box, so the ordinary border/text colors are the only
        // style inputs available to this renderer.
...
void HtmlApplication::SyncSelects() {
    if (!doc_ || !doc_->root()) return;
...
void HtmlApplication::SyncRanges() {
    if (!doc_ || !doc_->root()) return;
    ...
    for (const auto& range : doc_->root()->select_all("input[type=range]")) {
        ...
        set_text("#" + id + "-value", FormatRangeValue(current), TextUpdateMode::PaintOnly);
    }
}

void HtmlApplication::UpdateSelectFromMouse(const ImVec2& document_origin) {
...
void HtmlApplication::DrawSelects(const ImVec2& document_origin, const bool popup_only) {
    if (!doc_ || !doc_->root()) return;
    ImDrawList* draw_list = ImGui::GetWindowDrawList();
    if (!draw_list) return;
    ...
    if (hover_state_changed) {
        // set_attr() changes selector matching, but compute_styles(false) does
        // not register a selector that was absent when the option was first
        // styled. Reapply the stylesheet so the live data-hovered rule gets
        // its CSS background immediately, without rebuilding layout.
        doc_->refresh_styles();
    }
    ...
    for (std::size_t offset = 0; offset < popup.visible_count && first + offset < options.size(); ++offset) {
        ...
        if (is_selected) {
            // The option nodes are intentionally display:none: the popup is a
            // UA-style control surface painted by ImHTML. Selected state uses
            // the select's standard CSS accent-color.
            draw_list->AddRectFilled(row_min, row_max, option_selected_color, rounding);
        } else if (index == hovered) {
            // Non-selected hover remains an ordinary CSS background on the
            // hidden option node, just as a browser UA popup separates hover
            // feedback from the current selection.
...
void HtmlApplication::UpdateRangeFromMouse(const ImVec2& document_origin) {
    if (!doc_ || !doc_->root() || !doc_->root_render()) return;
    ...
    range->set_attr("data-range-value", serialized.c_str());
    range->set_attr("aria-valuenow", serialized.c_str());
    set_text("#" + active_range_id_ + "-value", FormatRangeValue(value), TextUpdateMode::PaintOnly);
...
void HtmlApplication::DrawRanges(const ImVec2& document_origin) const {
    if (!doc_ || !doc_->root()) return;
    ImDrawList* draw_list = ImGui::GetWindowDrawList();
    if (!draw_list) return;
    ...
    for (const auto& range : doc_->root()->select_all("input[type=range]")) {
        ...
        // `color` is the ordinary CSS color of the range input and gives the
        // thumb its independent color; `accent-color` remains the progress
        // color, matching the browser control model.
...
void HtmlApplication::MountLazyPanels() {
    if (!doc_ || !doc_->root()) return;

    // Appending a selected panel can reveal another lazy switcher. Repeat the
    // scan until the document contains no newly mounted active panel, which
    // keeps the convention useful for arbitrarily nested tab groups.
    ...
    while (mounted_panel) {
        ...
        for (const auto& switcher : switchers) {
            ...
            // The main content fragment is appended directly above. Nested
            // switchers all use the same lazy panel path.
...
std::shared_ptr<litehtml::element> HtmlApplication::NearestSwitcher(
...
bool HtmlApplication::IsElementMounted(const std::shared_ptr<litehtml::element>& element) const {
    for (auto current = element; current; current = current->parent()) {
...
std::string HtmlApplication::SwitchControlTarget(const std::shared_ptr<litehtml::element>& element) const {
    const std::string href = element->get_attr("href", "");
    return href.starts_with("#") ? href.substr(1) : std::string();
}

std::string HtmlApplication::SwitchPanelKey(const std::shared_ptr<litehtml::element>& element) const {
    return element->get_attr("id", "");
}

bool HtmlApplication::SwitchPanelMatches(const std::shared_ptr<litehtml::element>& element,
    ...
    // Keep select("group", "key") stable when the HTML uses a namespaced
    // panel id such as settings-panel-options.
...
void HtmlApplication::ApplySwitchers() const {
    if (!doc_ || !doc_->root()) return;
...
bool HtmlApplication::SelectPanelById(const std::string& id) {
...
bool HtmlApplication::ToggleCheckbox(const std::string& id) {
    ...
    // The switch knob is a pseudo-element whose horizontal position changes
    // with the checked state. This is a layout change, not paint-only state;
    // otherwise the track recolors but the pseudo-element keeps its old box.
...
void HtmlApplication::set_style(const std::string& selector, const std::string& css) {
    if (!doc_) {
        return;
    }
...
std::shared_ptr<litehtml::element> HtmlApplication::FindElement(const std::string& selector) {
    if (!doc_ || !doc_->root()) return {};
...
void HtmlApplication::set_flex_pixels(const std::string& selector, long pixels) {
    if (!doc_) return;
    const std::string css = "flex: 0 0 " + std::to_string(pixels) + "px;";
    const auto previous = inline_styles_.find(selector);
    if (previous != inline_styles_.end() && previous->second == css) return;

    if (auto target = FindElement(selector)) {
        // Keep the serialized inline style authoritative for a later full
        // style refresh, but update the already-computed values directly for
        // this geometry-only frame.
        target->set_attr("style", css.c_str());
        target->css_w().set_flex(0.0f, 0.0f, litehtml::css_length(static_cast<float>(pixels)));
        doc_->invalidate_layout_cache(target);
        inline_styles_[selector] = css;
        MarkDocumentLayoutDirty("app");
    }
...
void HtmlApplication::set_text(const std::string& selector, const std::string& text,
                               TextUpdateMode mode) {
    if (!doc_) return;
    ...
    if (auto target = FindElement(selector)) {
        ...
        if (mode == TextUpdateMode::PaintOnly && target->children().size() == 1) {
            const auto& child = *target->children().begin();
            if (child->is_text()) {
                const litehtml::size before = child->intrinsic_content_size();
                target->set_text(text.c_str());
                text_updated = true;
                const litehtml::size after = child->intrinsic_content_size();
                geometry_unchanged = before.width == after.width && before.height == after.height;
            }
        }

        if (!geometry_unchanged) {
            if (!text_updated) target->set_text(text.c_str());
            doc_->invalidate_layout_cache(target);
            MarkDocumentLayoutDirty("app");
        } else {
            ++paint_only_text_update_count_;
        }
...
// Nearest ancestor (inclusive) carrying the given ARIA role.
...
void CollectOptions(const std::shared_ptr<litehtml::element>& element,
    ...
    for (const auto& child : element->children()) {
        ...
        if (ElementRole(child) == "option") {
            out.push_back(child);
            // Options do not nest; skipping their subtree keeps document order
            // meaningful for shift-ranges.
            continue;
        }
...
bool HtmlApplication::apply_selection(const std::string& id, const bool ctrl_held,
                                      const bool shift_held) {
    if (!doc_ || id.empty()) return false;

    const auto clicked = FindElement("#" + id);
    // Form fields own their own interaction: the settings lists put text inputs
    // inside each row, and typing in one must not reselect the row and wipe the
    // rest of the selection. Buttons still select, which is what the file tiles
    // and similar item lists need.
    if (clicked) {
    ...
    // Some option-shaped controls are navigational rather than selectable
    // (for example folders in the content browser). They still use option
    // markup for a stable list layout, but must not replace file selections.
    ...
    // A listbox that never opted into multi-select collapses every modifier to a
    // plain click, which is what aria-multiselectable="false" means.
    ...
    if (!multi || (!ctrl_held && !shift_held)) {
        ...
        // Shift extends from the anchor; without one the click seeds it, which is
        // what happens when shift is the very first interaction with the list.
    ...
    // Only a click that replaces the selection may wipe the store. A virtualized
    // list has just the visible rows in the DOM, so a ctrl-toggle that rebuilt
    // the store from `options` would silently drop every selected row the user
    // has scrolled past.
...
// Re-applies the stored selection after a document rebuild. set_list() regenerates
// the list markup from template data, which resets aria-selected on every option;
// without this pass a click would appear to do nothing the moment anything
// republished the list.
void HtmlApplication::ApplyStoredAriaSelection() const {
...
std::vector<std::string> HtmlApplication::selected_ids(const std::string& listbox_id) {
...
void HtmlApplication::set_attribute(const std::string& selector, const std::string& name,
    ...
    if (auto target = FindElement(selector)) {
        ...
        // set_attr() changes selector matching, but compute_styles() alone
        // only recomputes selectors that were already registered on the
        // element. Refresh the live stylesheet state so newly matching
        // paint-only selectors (including pseudo-elements) are visible on
        // the very next draw, without rebuilding the render tree or layout.
        doc_->refresh_styles();
...
void HtmlApplication::frame() {
    ...
    // Without this, ImGui's default 8px WindowPadding insets the whole HTML
    // document from every edge, leaving a border of unpainted clear-color
    // pixels around the entire app.
    ImGui::PushStyleVar(ImGuiStyleVar_WindowPadding, ImVec2(0.0f, 0.0f));
    ImGui::Begin("HTML document", nullptr, flags);

    // A switcher click happens while the previous fragment is still being drawn.
    // Keep the active fragment/model unchanged for that traversal and commit the
    // selection at the start of the next frame.
    if (!pending_fragment_.empty()) {
        current_fragment_ = std::move(pending_fragment_);
        set_state("main-selection", current_fragment_);
    }

    // ParseDocument lays out immediately (queries
    // ImGui::GetContentRegionAvail() internally), so rebuilding must happen
    // inside this Begin/End, not before it.
    // Rows the previous frame decided are needed, applied before anything walks
    // the document again.
    ...
    // Custom ImGui components are drawn after the HTML document. Repaint only
    // the active native select popup last so panels such as NoorRay's
    // environment and renderer controls cannot cover the popup surface.
    DrawSelects(document_origin, true);

    // The rows are laid out now, so this is the only point where the list
    // geometry and the scroll offsets can be read against each other.
    UpdateListWindows();

    if (telemetry_enabled() && frame_count_ % 120 == 0) {
        std::fprintf(stderr,
                     "[ImHTML telemetry] frames=%llu rebuilds=%llu lists=%llu inactive-lists=%llu "
                     "paint-only-lists=%llu in-place-lists=%llu paint-only-text=%llu\n",
                     ...
                     static_cast<unsigned long long>(paint_only_text_update_count_));
    }
